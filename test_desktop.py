...
def test_price_filter_zero_values(page):
    ...
    if from_value == "0" and to_value == "0":
        page.wait_for_function(
            "window.location.search.includes('minPrice=0') && window.location.search.includes('maxPrice=0')"
        )
        assert (
            page.locator("._card_15fhn_2").count() == 0
            or page.locator("text=не найдены").is_visible()
        )
    else:
...
def test_stats_refresh_button(page):
    ...
    page.wait_for_selector("._refreshButton_ir5wu_16")

    page.get_by_role("button", name="Обновить").click()
    page.wait_for_function(
        "document.querySelector('._timeValue_ir5wu_112').textContent.includes('5:00')"
    )


def test_stats_stop_button(page):
    page.goto(BASE_URL)
    page.wait_for_timeout(1500)
    ...
    page.get_by_role("button", name="Отключить автообновление").click()

    page.wait_for_selector("text=Автообновление выключено")


def test_stats_start_button(page):
    page.goto(BASE_URL)
    page.wait_for_selector("._card_15fhn_2")

    page.get_by_role("link", name="Статистика").click()
    page.wait_for_selector("text=Управление таймером")

    page.get_by_role("button", name="Отключить автообновление").click()
    time_stopped = page.text_content("._refreshButton_ir5wu_16")

    page.get_by_role("button", name="Включить автообновление").click()
    page.wait_for_timeout(3000)
    time_after_start = page.text_content("._refreshButton_ir5wu_16")
    assert (
        time_stopped != time_after_start
    ), "Таймер не запустился (время не изменилось после 'Старт')"
